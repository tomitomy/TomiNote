...
type
  ...
  public
    ...
    procedure SetHistoryData(Data: TSteps; UpdateContent: Boolean=False;
...
procedure StrDiffFull(OldContent, NewContent: string; out SelStart1,
...
// Do not let external code and THistory write the same memory address
// 不要让外部代码和 THistory 写相同的内存地址
procedure THistory.SetHistoryData(Data: TSteps; UpdateContent: Boolean=False;
  const NewContent: string='');
begin
...
// Do not use the "const" keyword, this function will be used in another thread.
// 不要使用“const”关键字，这个函数会在另一个线程中使用。
procedure StrDiffFull(OldContent, NewContent: string; out SelStart1,
