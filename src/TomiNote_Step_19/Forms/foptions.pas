...
uses
  ...
  StdCtrls, ComCtrls, ExtCtrls, LCLTranslator;
...
type
  ...
  private
    ...
    function  LangNameToCode(LangName: string): string;
...
procedure TformOptions.LocalizationSpecialControl;
begin
  lstbTabs               .Items.Text := Res_OptionsTabsItems;
  chkgFullWindowHideBars .Items.Text := Res_HideBarsItems;
  chkgRememberWindowSize .Items.Text := Res_KeepWindowSizeItems;
...
begin
  ...
  chkgRememberWindowSize.Checked[5] := Config.KeepUtilsFormRect;
...
begin
  ...
  Config.KeepUtilsFormRect   := chkgRememberWindowSize.Checked[5];
  ...
  Config.LanguageCode:= LangNameToCode(combLanguage.Text);
end;

procedure TformOptions.LoadLangList;
var
  i: integer;
  LangCode, LangName: string;
  LangFile1, LangFile2: string;
begin
  FLangList.Delimiter := '|';
  FLangList.StrictDelimiter := True;
  FLangList.DelimitedText := Res_LangList;

  for i := 0 to FLangList.Count - 1 do begin
    LangCode := FLangList.Names[i];
    LangName := FLangList.ValueFromIndex[i];

    LangFile1 := ChangeFileExt(AppName, '.' + LangCode + '.po');
    LangFile2 := ChangeFileExt(AppName, '.' + LangCode + '.mo');

    if FileExists(ConcatPaths([formMain.LangDir, LangFile1])) or
      FileExists(ConcatPaths([formMain.LangDir, LangFile2])) then
        combLanguage.Items.Add(LangName);

    if LangCode = Config.LanguageCode then
      combLanguage.ItemIndex := combLanguage.Items.Count - 1;
  end;
end;

function TformOptions.LangNameToCode(LangName: string): string;
var
  i: integer;
begin
  Result := '';
  for i := 0 to FLangList.Count - 1 do begin
    if FLangList.ValueFromIndex[i] = LangName then begin
      Result := FLangList.Names[i];
      Break;
    end;
  end;
end;
